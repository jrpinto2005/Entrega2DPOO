...
public class CentralPersistencia {
			...
			throws FileNotFoundException, IOException, NumberFormatException, ParseException, ActivdadNoEcontradaException 
	{
		...
		// recorre el archivo hasta la ultima linea
		{
			...
			{
				RecursoEducativo recurso = new RecursoEducativo(partes[1], partes[2], partes[3], formato.parse(partes[4]),
						formato.parse(partes[5]), Integer.parseInt(partes[6]), Integer.parseInt(partes[7]),
						Double.parseDouble(partes[8]), "Recurso", Boolean.parseBoolean(partes[10]), partes[11]);
				sistema.addActividad(recurso);
				sistema.getLearningPaths().get(partes[11]).agregarActividad(recurso);
			}
			else if (partes[0].equals("tarea"))
			{
				Tarea tarea = new Tarea(partes[1], partes[2], partes[3], formato.parse(partes[4]),
						formato.parse(partes[5]), Integer.parseInt(partes[6]), Integer.parseInt(partes[7]),
						Double.parseDouble(partes[8]), "Tarea", Boolean.parseBoolean(partes[10]), partes[11]);
				sistema.addActividad(tarea);
				sistema.getLearningPaths().get(partes[11]).agregarActividad(tarea);
			}
